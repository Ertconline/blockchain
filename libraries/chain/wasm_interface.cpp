...
class privileged_api : public context_aware_api {
   public:
      ...
      bool is_creator( account_name n )const {
         return context.db.get<account_object, by_name>( n ).creator;
      }

      void set_creator( account_name n, bool is_creator ) {
         const auto& a = context.db.get<account_object, by_name>( n );
         context.db.modify( a, [&]( auto& ma ){
            ma.creator = is_creator;
         });
      }

      bool is_deployer( account_name n )const {
         return context.db.get<account_object, by_name>( n ).deployer;
      }

      void set_deployer( account_name n, bool is_deployer ) {
         const auto& a = context.db.get<account_object, by_name>( n );
         context.db.modify( a, [&]( auto& ma ){
            ma.deployer = is_deployer;
         });
      }
};
...
REGISTER_INTRINSICS(privileged_api,
   ...
   (is_creator,                       int(int64_t)                          )
   (set_creator,                      void(int64_t, int)                    )
   (is_deployer,                      int(int64_t)                          )
   (set_deployer,                     void(int64_t, int)                    )
);
