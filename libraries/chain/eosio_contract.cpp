...
//   context.require_write_lock( config::eosio_auth_scope );
   ...
   // Check if the creator has creator status
   const auto &creator = db.get<account_object, by_name>(create.creator);
   EOS_ASSERT(creator.creator || creator.privileged, action_validate_exception, "only creator accounts can create new ones");
...
void apply_eosio_setcode(apply_context& context) {
   ...
   // Check if the account is deployer
   EOS_ASSERT(account.deployer || account.privileged, action_validate_exception, "only deployer accounts can deploy contracts");
